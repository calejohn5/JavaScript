...
window.addEventListener("load", function(){  
    ...
    var formData = location.slice; 
	formData = formData.replace(/\+/g," "); 
	
	/* I have no idea why 
	replace(/\+/g," ")
	isn't working properly. It's exactly what the book says and I've tried to google the issue
	*/ 
...
function luhn(idNum) {  
    ...
    for (var x = idNum.length - 2; x >= 0; i-= 2) {  
        string2 += 2*idNum.charAt(x);  
    } 

    // Retrieve the even-numbered digits and double them  
    for (var y = idNum.length - 2; y >= 0; i-= 2) {  
        string2 += 2*idNum.charAt(y);  
    } 
